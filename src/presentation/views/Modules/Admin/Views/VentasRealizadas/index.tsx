...
import ModalMotivo from "./motivo";
...
export const VentasRealizadas = () => {
  ...
  const [modalOpenMotivo, setModalOpenMotivo] = useState<boolean>(false);
  const [idComprobante, setComprobanteId] = useState<number>(0);
  const [loadingVentas, setLoadingVentas] = useState<boolean>(true);

  const newDataAnfitrionas = ventas?.map((value: any) => {
    return {
      ...value,
      ...
      tipoDocumento: value?.tipoDocumentoVentaId === 1 ? "factura" : value?.tipoDocumentoVentaId === 2 ? "Boleta" : "Ticket Interno",
    };
  });

  const verProductosMain = (data: any) => {
    dispatch(openModalVentas());
    dispatch(activeVentas(data));
  };
  const anularVentasMain = (data: any) => {
    setComprobanteId(data?.idComprobante)
    setModalOpenMotivo(true)
    // dispatch(AnularVenta(data?.idComprobante));
  };
  ...
  useEffect(() => {
    ...
    dispatch(getAllVentas(dateStart,dateEnd)).finally(() => setLoadingVentas(false));
  }, [dateStart, dispatch]);
  useEffect(() => {
    printTable(`${title.name}::VENTAS DEL DIA`);
  }, []);
  useEffect(() => {
  ...
  return (
    <>
      {modalOpenMotivo && <ModalMotivo onClose={onClose} id={idComprobante} />}
